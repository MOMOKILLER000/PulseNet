...
export default function PulseDetails() {
    ...
    if (loading) return <div className={styles.loading}>Loading pulse...</div>;
    ...
    const commentStyles = {
        ...
        commentBox: { borderBottom: '1px solid #eee', paddingBottom: 8 },
                            ...
                            <div style={{ marginTop: '40px', borderTop: '1px solid #eee', paddingTop: '20px' }}>
                                ...
                                {showComments && (
                                    <div style={{ marginTop: 18 }}>
                                        ...
                                        {!commentsLoading && comments.length === 0 && <div style={{ color: '#666' }}>No comments yet.</div>}

                                        <div style={commentStyles.container}>
                                            {comments.map((c) => (
                                                <div key={c.id || `${c.user}-${c.pub_date}`} style={commentStyles.commentBox}>
                                                    <div style={commentStyles.header}>
                                                        <strong>{c.user || (c.user_username ?? c.user_name)}</strong>
                                                        <small style={{ color: '#999' }}>{c.date ?? c.pub_date ?? ""}</small>
                                                    </div>
                                                    <p style={commentStyles.content}>{c.content}</p>
                                        ...
                                        {commentsHasMore && (
                                            <div style={{ marginTop: 10, textAlign: 'center' }}>
                                                <button onClick={handleLoadMoreComments} disabled={commentsLoading} style={{ padding: '8px 12px', borderRadius: 8 }}>
